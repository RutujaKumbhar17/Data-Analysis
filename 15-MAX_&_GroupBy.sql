...
-- MAX-> Gives maximum of all the elements.Can also be used on text data.
select max (totalamount) from dbo.sales -- this gives max but column name doesn't appear.

select max (totalamount ) [Maximum Amount] from dbo.Sales -- here we gave a column name.
...
-- Maximum Toatal Amount for all distinct dates in saledate column.
